...
@section('content')
   <div class="flex flex-col h-screen main-background items-center justify-center py-8">
    ...
    <div class="max-w-lg md:max-w-4xl p-8 bg-white ">
        <form class="flex flex-col gap-6" method="POST" action="{{ route('user.update', $user->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="flex flex-col gap-6">
                    <div>
                        <x-forms.input label="Nome Completo" id="name" name="name" type="text"
                            placeholder="Ex: João da Silva" value="{{ old('name', $user->name) }}" />
                        @error('name')
                    ...
                    <div>
                        <x-forms.input label="CPF" id="cpf" name="cpf" type="text"
                            placeholder="Ex: 000.000.000-00" value="{{ old('cpf', $user->cpf) }}" />
                        @error('cpf')
                            <span class="text-red-500 text-xs ">{{ $message }}</span>
                        @enderror
                    </div>
                   <div>
                     <x-forms.input label="CEP" id="cep" name="cep" type="text"
                        placeholder="Ex: 00000-000" value="{{ old('cep', $user->cep) }}" />
                    @error('cep')
                        <span class="text-red-500 text-xs ">{{ $message }}</span>
                    @enderror
                   </div>
                   <div>
                       <x-forms.input label="Bairro" id="neighborhood" name="neighborhood" type="text"
                           placeholder="Ex: Centro" value="{{ old('neighborhood', $user->neighborhood) }}" />
                       @error('neighborhood')
                           <span class="text-red-500 text-xs ">{{ $message }}</span>
                       @enderror
                   </div>
